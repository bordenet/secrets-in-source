...
$ cd cc_deploy/extras/secrets_in_source
...
**Concrete examples:**
```bash
pushd cc_deploy/extras/secrets_in_source > \dev\null
go run ./passhog.go ../.. --types=cs,py --output=passhog_results_cs_py.txt
...
pushd test
go run ./secrets-gap-analysis.go -t ../trufflehog_all.json -p ../passhog_all.txt
popd
```
## Improve it
